...
const DEBUG_AUTH = index_js_1.config.debugAuth;
const REFRESH_COOKIE = index_js_1.config.jwt.refreshCookie;
...
router.post('/login', async (req, res) => {
    ...
    `;
        ...
        const access = await (0, tokenService_js_1.signAccess)(userId);
        const refresh = await (0, tokenService_js_1.signRefresh)(userId, 0);
        // HttpOnly Refresh Cookie (クロスサイト運用)
        res.setHeader('Set-Cookie', [
            `${REFRESH_COOKIE}=${encodeURIComponent(refresh)}; ${COOKIE_BASE}; Max-Age=${index_js_1.config.jwt.refreshTtlSec}`,
        ]);
        return res.status(200).json({
            ok: true,
            access_token: access,
            token_type: 'Bearer',
            expires_in: index_js_1.config.jwt.accessTtlSec,
...
router.post('/refresh', async (req, res) => {
    try {
        const rt = (0, tokenService_js_1.readCookie)(req, REFRESH_COOKIE);
        if (!rt)
            return res.status(401).json({ error: 'no_refresh_token' });
        const { payload } = await (0, tokenService_js_1.verifyRefresh)(rt);
        const uidNorm = normalizeUid(payload.uid);
        if (uidNorm == null)
            return res.status(401).json({ error: 'invalid_refresh_uid' });
        // （必要ならここでリフレッシュの失効/回転チェック）
        const access = await (0, tokenService_js_1.signAccess)(uidNorm);
        return res.json({
            ok: true,
            access_token: access,
            token_type: 'Bearer',
...
router.get('/me', async (req, res) => {
    try {
        ...
        const uidNorm = normalizeUid(payload.uid);
        if (uidNorm == null) {
            console.warn('[auth/me] invalid uid in access token:', payload.uid);
            return res.status(401).json({ error: 'unauthenticated' });
        }
        console.log('[auth/me] uid=', uidNorm);
        const db = req.app.locals.db;
        const r = await db.query(`SELECT u.id, u.line_user_id,
              p.nickname, p.age, p.gender, p.occupation, p.photo_url, p.photo_masked_url, p.verified_age
       FROM users u
       LEFT JOIN user_profiles p ON p.user_id = u.id
       WHERE u.id = $1`, [uidNorm]);
        if (!r.rows[0])
