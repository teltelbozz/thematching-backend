...
router.get('/', async (req, res) => {
    try {
        ...
        const r = await db.query(`SELECT user_id, participation_style, party_size, desired_date,
...
router.put('/', async (req, res) => {
    try {
        ...
        if (uid == null)
            return res.status(401).json({ error: 'unauthenticated' });
        const { participation_style, party_size, desired_date, // "YYYY-MM-DD"
        type_mode, venue_pref, cost_pref, } = req.body || {};
        ...
        const db = req.app.locals.db;
        await db.query(`INSERT INTO user_match_setup (
