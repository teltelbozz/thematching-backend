...
const matchEngine_1 = require("./matchEngine"); // Step4で作ったロジック
/**
 * 翌日スロットのマッチングを実行する。
 *
 ...
 */
async function runDailyMatching(db) {
    // バッチ実行日の JST 今日
    const now = new Date();
    ...
    for (const row of slotRes.rows) {
      ...
      `, [slot_dt]);
        ...
        for (const [key, users] of map.entries()) {
            ...
            // マッチングロジック実行
            const { groups, unmatched } = (0, matchEngine_1.runMatching)(users);
            console.log(`[matchService] slot=${slot_dt} loc=${location} type=${type_mode} ⇒ groups: ${groups.length}`);
            // 保存
            const createdIds = [];
          ...
          `, [slot_dt, location, type_mode]);
                ...
                for (const f of g.female_ids) {
                    await db.query(`INSERT INTO matched_group_members (group_id, user_id, gender)
             VALUES ($1, $2, 'female')`, [groupId, f]);
                }
                for (const m of g.male_ids) {
                    await db.query(`INSERT INTO matched_group_members (group_id, user_id, gender)
             VALUES ($1, $2, 'male')`, [groupId, m]);
                }
                // 履歴登録（再マッチ禁止用）
                for (const f of g.female_ids) {
                    for (const m of g.male_ids) {
                        await db.query(`INSERT INTO match_history (user_id_female, user_id_male, slot_dt)
               VALUES ($1, $2, $3)
               ON CONFLICT DO NOTHING`, [f, m, slot_dt]);
                    }
                }
            }
            results.push({
