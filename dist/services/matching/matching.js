"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.runMatchingForSlot = runMatchingForSlot;
const createGroup_1 = require("./createGroup");
const history_1 = require("./history");
const score_1 = require("./score");
const score_2 = require("./score");
const history_2 = require("./history");
/**
 * バッチのメイン処理
 */
async function runMatchingForSlot(params) {
    const { db, slotDt } = params;
    // =====================================================
    // 1. この slotDt に関連するユーザー（エントリ済み）を取得
    // =====================================================
    const entrySql = `
    SELECT
      us.user_id,
      up.gender,
      up.age,
      us.type_mode,
      us.location
    FROM user_setup us
    JOIN user_setup_slots sl ON sl.user_setup_id = us.id
    JOIN user_profiles up ON up.user_id = us.user_id
    WHERE sl.slot_dt = $1
  `;
    const entryRes = await db.query(entrySql, [slotDt]);
    const rows = entryRes.rows;
    // 男女分け
    const females = rows.filter(r => r.gender === 'female');
    const males = rows.filter(r => r.gender === 'male');
    // 男女とも 2 名未満 → マッチング不可
    if (females.length < 2 || males.length < 2) {
        return {
            slotDt,
            totalCandidates: 0,
            chosenGroups: 0,
            unmatched: females.length + males.length,
            groups: [],
        };
    }
    // =====================================================
    // 2. 女性2名組 / 男性2名組 の全通りを作成
    // =====================================================
    function pairs(arr) {
        const out = [];
        for (let i = 0; i < arr.length; i++) {
            for (let j = i + 1; j < arr.length; j++) {
                out.push([arr[i], arr[j]]);
            }
        }
        return out;
    }
    const fPairs = pairs(females);
    const mPairs = pairs(males);
    // =====================================================
    // 3. 履歴読み込み（男女1対1禁止 / 同性は許容）
    // =====================================================
    const historySql = `
    SELECT user_id_female, user_id_male
    FROM match_history
  `;
    const historyRes = await db.query(historySql);
    const history = new Set(historyRes.rows.map(r => `${r.user_id_female}-${r.user_id_male}`));
    const candidates = [];
    for (const fp of fPairs) {
        for (const mp of mPairs) {
            // 再マッチ禁止（男女の1:1 組み合わせすべて確認）
            if ((0, history_2.violatesRematch)(fp.map(f => f.user_id), mp.map(m => m.user_id), history)) {
                continue;
            }
            // スコア計算
            const score = (0, score_2.groupAgeScore)(fp.map(f => ({ id: f.user_id, age: Number(f.age) })), mp.map(m => ({ id: m.user_id, age: Number(m.age) })));
            if (score >= score_1.SCORE_THRESHOLD) {
                const ages = fp.map(f => Number(f.age)).sort((a, b) => a - b);
                const medianAge = ages[1]; // 2人組なので2番目
                candidates.push({
                    fp: fp.map(f => f.user_id),
                    mp: mp.map(m => m.user_id),
                    score,
                    tie: medianAge,
                });
            }
        }
    }
    // =====================================================
    // 5. スコア降順 → tie（女性中央値）昇順
    // =====================================================
    candidates.sort((a, b) => {
        if (a.score !== b.score)
            return b.score - a.score;
        return a.tie - b.tie;
    });
    // =====================================================
    // 6. Greedyに非重複グループを確定
    // =====================================================
    const used = new Set();
    const chosen = [];
    for (const c of candidates) {
        const users = [...c.fp, ...c.mp];
        // 4人すべて未使用なら採択
        if (users.every(uid => !used.has(uid))) {
            chosen.push(c);
            users.forEach(uid => used.add(uid));
        }
    }
    // =====================================================
    // 7. グループを DB に保存 (createMatchedGroup)
    // =====================================================
    const results = [];
    for (const c of chosen) {
        const group = await (0, createGroup_1.createMatchedGroup)({
            db,
            slotDt,
            location: 'shibuya_shinjuku',
            typeMode: 'wine_talk', // type_mode は女性側に合わせるなど後続仕様
            fPair: c.fp,
            mPair: c.mp,
        });
        // 男女1対1 の履歴を登録
        await (0, history_1.addHistory)(db, c.fp, c.mp, slotDt);
        results.push({
            score: c.score,
            female: c.fp,
            male: c.mp,
            groupId: group.id,
            token: group.token,
        });
    }
    // =====================================================
    // 8. 未成立数
    // =====================================================
    const allUserIds = rows.map(r => r.user_id);
    const unmatched = allUserIds.filter(uid => !used.has(uid));
    return {
        slotDt,
        totalCandidates: candidates.length,
        chosenGroups: chosen.length,
        unmatched: unmatched.length,
        groups: results,
    };
}
