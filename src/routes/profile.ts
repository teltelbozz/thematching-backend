...
// Body: { nickname?, age?, gender?, occupation?, photo_url?, photo_masked_url? }
router.put('/', async (req, res) => {
  try {
    ...
    if (nickname && typeof nickname !== 'string') return res.status(400).json({ error: 'invalid_nickname' });
    if (age != null && !(Number.isInteger(age) && age >= 18 && age <= 120)) return res.status(400).json({ error: 'invalid_age' });
    if (gender && typeof gender !== 'string') return res.status(400).json({ error: 'invalid_gender' });
    if (occupation && typeof occupation !== 'string') return res.status(400).json({ error: 'invalid_occupation' });
    if (photo_url && typeof photo_url !== 'string') return res.status(400).json({ error: 'invalid_photo_url' });
    if (photo_masked_url && typeof photo_masked_url !== 'string') return res.status(400).json({ error: 'invalid_photo_masked_url' });

    const db = req.app.locals.db as Pool;
    // user_profiles が無ければ作る → あれば更新
