...
async function resolveUserIdFromToken(db: Pool, uidClaim: unknown): Promise<number | null> {
  // 1) すでに数値ならそのまま
  if (typeof uidClaim === 'number' && Number.isFinite(uidClaim)) return uidClaim;
...
router.get('/', async (req, res) => {
  try {
    ...
    const userId = await resolveUserIdFromToken(db, (payload as any).uid);
    ...
    if (!r.rows[0]) return res.json({ profile: { id: userId } }); // 未登録でも200で空を返す
